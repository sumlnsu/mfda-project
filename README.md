...
`reports/`에는 MFDA 분석 결과를 정리한 리포트 산출물이 들어 있습니다.

- `mfda_report.pdf`: 남성 패션 데이터 분석 종합 리포트
- `lmc_trend_analysis_report.pdf`: MFDA 분석 인사이트와 대시보드 플랫폼을 활용한 LMC 브랜드 트렌드 분석 리포트

이 리포트들은 포트폴리오에서 다음 메시지를 전달하기 위한 산출물입니다.
